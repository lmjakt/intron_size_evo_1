...
## This script outputs intron and exon data for all genes from each
## species included. The family_members file is read only to get the
## names of the databases, and to map these to species names.

## call this with something like
## ./compile_all_seqs.pl vertebrate_family_members_1_15.txt genome_list
...
print STDERR "Getting list of sequence files from $seq_file_list\n";
...
open(my $stats, ">", "all_genes/exon_intron_stats.csv") || die "unable to open exon_intron_stats.csv $!\n";

for $i(0..$#db_names){
    my $sp_name = $sp_names[$i];
##    %{$gene_data{$sp_name}} = get_filtered_table($gene_transcript_query, $db_names[$i]);
    %gene_data = get_filtered_table($gene_transcript_query, $db_names[$i]);
##    %{$gene_data{$sp_name}} = get_filtered_table($gene_transcript_query, $db_names[$i], 0, $gene_ids{$db_names[$i]});
    print STDERR "$db_names[$i] : ", scalar( @{$gene_data{data}} ), "  rows\n";
    ## create a map of gene identifiers to row numbers in the given table.
    %gene_exon = ();
    for $j(0..$#{$gene_data{data}}){
	push @{$gene_exon{ $gene_data{data}[$j][0] }}, $j;
    }
    extract_sequences($sp_name);
}


## and at this point we have everything that we need apart from the sequences from the different genomes.

## since we don't care about family membership here we can make a single fasta file for each species
## from which we can extract the relevant data.


sub extract_sequences {
    $sp = shift @_;
## for $sp(keys %gene_data){
    print STDERR "getting sequences from genomes: $sp\n";
    ## let us make a map of the columns of the gene data set
    my %col_i = ();
    for my $j( 0..$#{$gene_data{fields}} ){
	$col_i{ $gene_data{fields}[$j] } = $j;
    }

    my @gids = keys %gene_exon;
    my %seq = read_fasta( $seq_map{$sp} );
    print STDERR "Genome sequence read\n";
    my $exon_file = "all_genes/".$sp."_exons.fa";
    my $intron_file = "all_genes/".$sp."_introns.fa";
    open(EXON, ">", $exon_file) || die "unable to open $exon_file for writing $!";
    open(INTRON, ">", $intron_file) || die "unable to open $intron_file for writing $!";
...
#	    ;
    for my $gid(@gids){
	my @j = @{$gene_exon{ $gid }};
	my $chr = $gene_data{data}[$j[0]][ $col_i{'chr'} ];
	next if( !defined($seq{$chr}) ); ## avoid alternative chromosomes
	my $header = ">".$gene_data{data}[$j[0]][$col_i{'gene'}]."\t".$gene_data{data}[$j[0]][$col_i{'transcript'}].
	    "\t".$gene_data{data}[$j[0]][ $col_i{'chr'} ]." ".$gene_data{data}[$j[0]][$col_i{'start'}].
	    "..".$gene_data{data}[$j[0]][$col_i{'end'}].":".$gene_data{data}[$j[0]][$col_i{'strand'}].
	    "\t".$db_map{$sp}."\n";
	print EXON $header;
	print INTRON $header;
	print $stats $sp, "\t", $db_map{$sp}, "\t", $gene_data{data}[$j[0]][$col_i{'gene'}], "\t",
	    $gene_data{data}[$j[0]][$col_i{'transcript'}], "\t", $gene_data{data}[$j[0]][ $col_i{'chr'} ],
	    "\t", $gene_data{data}[$j[0]][$col_i{'strand'}], "\t", $gene_data{data}[$j[0]][$col_i{'start'}], "..",
	    $gene_data{data}[$j[0]][$col_i{'end'}], "\t";
	my $last_stop = -1;
	my @e_sizes = ();
	my @i_sizes = ();
	for my $k( 0..$#j ){
...
#	    my $chr = $gene_data{data}[$j][ $col_i{'chr'} ];
	    my $start = $gene_data{data}[$j][ $col_i{'e.start'} ];
	    my $stop =  $gene_data{data}[$j][ $col_i{'e.end'} ];
	    my $strand = $gene_data{data}[$j][ $col_i{'strand'} ];
