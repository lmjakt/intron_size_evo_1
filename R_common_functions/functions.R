...
## this is necessary if we wish to calculate n50 etc..
get.assembly <- function(db.name){
...
get.chr.regions <- function(db.name, exclude=NULL){
    query <- paste("select a.* from seq_region a",
                   "inner join coord_system b",
                   "on a.coord_system_id=b.coord_system_id and b.rank=1");
    if(!is.null(exclude))
        query <- paste(query, " where a.name not regexp '", exclude, "'", sep='')
